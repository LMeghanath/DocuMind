...
def upload_docs(request):
    if request.method == "POST":
        ...
        user_folder = os.path.join(settings.MEDIA_ROOT, "documents", str(user_id))
...
def delete_all_docs(user):
    ...
    store_dir = get_faiss_store_dir(user.id)
...
def delete_doc(request, id):
    ...
    faiss_file = os.path.join(store_dir, f"{doc.id}.faiss")
    pkl_file = os.path.join(store_dir, f"{doc.id}.pkl")
    meta_file = os.path.join(store_dir, f"{doc.id}_meta.json")
